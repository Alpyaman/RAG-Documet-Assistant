    ...
    def _chunk_by_paragraph(self, document: Document) -> List[Chunk]:
        ...
        for paragraph in paragraphs:
            ...
            if para_len > self.chunk_size:
                ...
                # --- FIX: RE-INDEX CHUNKS TO AVOID COLLISIONS ---
                for p_chunk in para_chunks:
                    # Overwrite the ID generated by fixed_size to use our current sequence
                    p_chunk.chunk_id = f"{document.metadata.get('filename', 'doc')}_{chunk_index}"
                    p_chunk.metadata["chunk_index"] = chunk_index
                    
                    # Update character positions to be relative to the full document
                    p_chunk.start_char += char_position
                    p_chunk.end_char += char_position
                    
                    chunk_index += 1
                
                chunks.extend(para_chunks)
                
                # Don't forget to advance char_position for the big paragraph
