...
def query_documents(query: str, top_k: int = 5) -> Optional[Dict]:
    ...
    try:
        response = requests.post(
            f"{API_BASE_URL}/query", json={"query": query, "top_k": top_k}
        )
        response.raise_for_status()
        return response.json()
    except requests.exceptions.RequestException as e:
...
def main():
    ...
    st.markdown(
        '<div class="sub-header">Upload PDFs and chat with your documents using AI</div>',
        unsafe_allow_html=True,
    )
    ...
    if not check_api_health():
        st.error(
            "⚠️ Cannot connect to the API backend. Please ensure the FastAPI server is running."
        )
    ...
    with tab1:
        ...
        st.markdown(
            "Ask questions and get AI-generated answers based on your uploaded documents."
        )
            ...
            if not user_query:
                ...
                with st.spinner("Generating answer..."):
                    result = generate_answer(
                        user_query, top_k=top_k, return_context=True
                    )
                    ...
                    if result:
                        ...
                        if result.get("context_used"):
                            with st.expander(
                                f"📄 View {len(result['context_used'])} Context Sources"
                            ):
        ...
        if st.session_state.chat_history:
            ...
            for idx, chat in enumerate(reversed(st.session_state.chat_history[-5:])):
                with st.container():
                    st.markdown(
                        f"**Q{len(st.session_state.chat_history) - idx}:** {chat['query']}"
                    )
    ...
    with tab2:
        ...
        st.markdown(
            "Search for relevant passages without AI generation (pure retrieval)."
        )
